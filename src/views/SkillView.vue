<template>
  <div
    class="container grid grid-cols-3 grid-rows-1 h-full text-textColor gap-x-10"
  >
    <div
      class="bg-blue h-full bg-slate-700 overflow-auto order-1 shadow-md shadow-contrast hover:shadow-lg hover:shadow-contrast transition-shadow ease-in-out rounded-lg"
    >
    ...
    <div
      class="bg-blue h-full bg-slate-700 overflow-auto order-2 shadow-md shadow-contrast hover:shadow-lg hover:shadow-contrast transition-shadow ease-in-out rounded-lg"
    >
      <div class="px-5 h-12 bg-primary">
    ...
    <div
      class="bg-blue h-full bg-slate-700 overflow-auto order-3 shadow-md shadow-contrast hover:shadow-lg hover:shadow-contrast transition-shadow ease-in-out rounded-lg"
    >
      <div class="px-5 h-12 bg-primary">
        <h1>Options</h1>
      </div>
      ...
      <div>
        ...
        <div>
          <input
            ...
            id="huey"
            name="mode"
            :value="CalculationMode.Basic"
            checked
          />
          <label for="huey">Basic</label>
        </div>
        ...
        <div>
          <input
            ...
            id="dewey"
            name="mode"
            :value="CalculationMode.Even"
            disabled
          />
          <label for="dewey">Even</label>
        </div>
        ...
        <div>
          <input
            ...
            id="louie"
            name="mode"
            :value="CalculationMode.Test"
            disabled
          />
          <label for="louie">Test</label>
        </div>
      </div>
      <div>Materials</div>
      <div>Profit</div>

      <div>Gained XP: {{ gainedXP }}</div>
    </div>
  </div>
</template>
...
const mode: Ref<CalculationMode> = ref(CalculationMode.Basic);
const test = ref(0);
...
function calculateBasic() {
  ...
  for (const key in doneItems.value) {
    gainedXP.value +=
      doneItems.value[key].amount * doneItems.value[key].experience["Smithing"];
  }
}
